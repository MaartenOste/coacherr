...
const Bench = ({amount, arrayOfPlayers, update}) => {
	...
	const createRow = () => {
		...
		let temp = [];
		for (let i = 11; i < 16; i++) {
			temp.push(<div className="playerContainer" id={i} key={i} onClick={ev=> changePlayer(ev.target)}>{rowpl[i] && rowpl[i].lastname !== 'Add'?<i className="fas fa-tshirt noclick"></i>:<i className="fas fa-plus-circle noclick"></i>}<div className="playerName noclick">{rowpl[i]?rowpl[i].lastname.length>6?(rowpl[i].lastname.substring(0, 5) + "..."):rowpl[i].lastname:'add'}</div></div>)
		}
		setRow(temp);
	}
